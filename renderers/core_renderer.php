...
/**
 * essential_gourmet theme with the underlying Bootstrap theme.
 *
 * @package    theme
 * @subpackage essential_gourmet
 ...
 */
 ...
 class theme_essential_gourmet_core_renderer extends theme_bootstrapbase_core_renderer {
    ...
    public function footer() {
        ...
        if (defined('MDL_PERF') || (!empty($CFG->perfdebug) and $CFG->perfdebug > 7)) {
            ...
            if (defined('MDL_PERFTOFOOT') || debugging() || $CFG->perfdebug > 7) {
                $performanceinfo = essential_gourmet_performance_output($perf);
            }
        }
    ...
    protected function render_custom_menu(custom_menu $menu) {
    	...
    	*/
        ...
        if (isloggedin() && !isguestuser() && $hasdisplaymycourses) {
            ...
            if ($mycoursetitle == 'module') {
				$branchtitle = get_string('mymodules', 'theme_essential_gourmet');
			} else if ($mycoursetitle == 'unit') {
				$branchtitle = get_string('myunits', 'theme_essential_gourmet');
			} else if ($mycoursetitle == 'class') {
				$branchtitle = get_string('myclasses', 'theme_essential_gourmet');
			} else {
				$branchtitle = get_string('mycourses', 'theme_essential_gourmet');
			}
            ...
            $branch = $menu->add($branchlabel, $branchurl, $branchtitle, $branchsort);
 			if ($courses = enrol_get_my_courses(NULL, 'fullname ASC')) {
                ...
                $noenrolments = get_string('noenrolments', 'theme_essential_gourmet');
    	...
    	*/
        ...
        if (isloggedin() && !isguestuser() && $hasdisplaymydashboard) {
            $branchlabel = '<i class="fa fa-dashboard"></i>'.get_string('mydashboard', 'theme_essential_gourmet');
            $branchurl   = new moodle_url('/my/index.php');
            $branchtitle = get_string('mydashboard', 'theme_essential_gourmet');
            $branchsort  = 10000;
    ...
    */
    ...
    public function essential_gourmetblocks($region, $classes = array(), $tag = 'aside') {
...
class theme_essential_gourmet_format_topics_renderer extends format_topics_renderer {
    ...
    protected function get_nav_links($course, $sections, $sectionno) {
        ...
        while ($back > 0 and empty($links['previous'])) {
            if ($canviewhidden || $sections[$back]->uservisible) {
                ...
                $previouslink .= get_string('previoussection', 'theme_essential_gourmet');
        ...
        while ($forward <= $course->numsections and empty($links['next'])) {
            if ($canviewhidden || $sections[$forward]->uservisible) {
                ...
                $nextlink .= get_string('nextsection', 'theme_essential_gourmet');
