...
/**
 * The essential_gourmet theme is built upon the Bootstrapbase theme.
 *
 * @package    theme
...
if ($hasmarketing1image) {
    $marketing1image = $PAGE->theme->setting_file_url('marketing1image', 'marketing1image');
}
if ($hasmarketing2image) {
    $marketing2image = $PAGE->theme->setting_file_url('marketing2image', 'marketing2image');
}
if ($hasmarketing3image) {
    $marketing3image = $PAGE->theme->setting_file_url('marketing3image', 'marketing3image');
}
...
<head>
    ...
    <noscript>
			<link rel="stylesheet" type="text/css" href="<?php echo $CFG->wwwroot;?>/theme/essential_gourmet/style/nojs.css" />
	</noscript>
    <!-- Google web fonts -->
...
<?php if($PAGE->theme->settings->usefrontcontent ==1) {
  ?> <section class="promo box box-dark"> <?php
...
<!-- End Marketing Spots -->
	...
	<div class="hidden-blocks">
    	<div class="row-fluid">
        	<h4><?php echo get_string('visibleadminonly', 'theme_essential_gourmet') ?></h4>
            <?php
                echo $OUTPUT->essential_gourmetblocks('hidden-dock');
            ?>
    	</div>
	</div>
	<?php } ?>
